...
function searchList(list, queary) {
    function searchcallback(string) {
        return string.toLowerCase().includes(queary.toLowerCase());
    }  
...
function searchList2(list, queary) {
    function searchcallback(item) {
        return (
        item.name.toLowerCase().includes(queary.toLowerCase()) || //the || means or
        item.description.toLowerCase().includes(queary.toLowerCase()) ||
        item.tags.find().toLowerCase().includes(queary.toLowerCase())
        );
    }  
    return list.filter(searchcallback);
}
